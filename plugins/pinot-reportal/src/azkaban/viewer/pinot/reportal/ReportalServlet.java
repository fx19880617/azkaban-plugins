...
public class ReportalServlet extends LoginAbstractAzkabanServlet {
  ...
  private void handleListReportal(HttpServletRequest req, HttpServletResponse resp, Session session) throws ServletException, IOException {
    ...
    Page page = newPage(req, resp, session, "azkaban/viewer/pinot/reportal/reportallistpage.vm");
  ...
  private void handleViewReportal(HttpServletRequest req, HttpServletResponse resp, Session session) throws ServletException, Exception {
    int id = getIntParam(req, "id");
    Page page = newPage(req, resp, session, "azkaban/viewer/pinot/reportal/reportaldatapage.vm");
    preparePage(page, session);
    ...
    if (hasParam(req, "execid")) {
      ...
      if (hasParam(req, "logs")) {
        ...
        // List files
          ...
          if (jobLogs.size() == 1) {
            resp.sendRedirect("/pinot-reportal?view&logs&id=" + project.getId() + "&execid=" + execId + "&log=" + jobLogs.get(0).getId());
          }
  ...
  private void handleRunReportal(HttpServletRequest req, HttpServletResponse resp, Session session) throws ServletException, IOException {
    int id = getIntParam(req, "id");
    ProjectManager projectManager = server.getProjectManager();
    Page page = newPage(req, resp, session, "azkaban/viewer/pinot/reportal/reportalrunpage.vm");
    preparePage(page, session);
  ...
  private void handleNewReportal(HttpServletRequest req, HttpServletResponse resp, Session session) throws ServletException, IOException {
    ...
    Page page = newPage(req, resp, session, "azkaban/viewer/pinot/reportal/reportaleditpage.vm");
    preparePage(page, session);
  ...
  private void validatePinotConfigs(List<Variable> pinotConfigs) throws Exception
  {
    String[] requiredPinotConfigList = {"pinot-data-input-path", "pinot-segment-output-path", "pinot-cluster-name", "pinot-dataset-name", "pinot-push-fabric", "pinot-time-column-name", "pinot-time-column-type"};
    for (Variable pinotConfig : pinotConfigs) {
      if (Arrays.binarySearch(requiredPinotConfigList, pinotConfig.title) >= 0) {
        if (pinotConfig.name.isEmpty()) {
          throw new Exception(requiredPinotConfigList[Arrays.binarySearch(requiredPinotConfigList, pinotConfig.title)].replace("-", " ") + " must not be empty.");
        }
      }
      if (pinotConfig.title.equalsIgnoreCase("pinot-primary-key-columns")) {
        if (pinotConfig.name.isEmpty()) {
          throw new Exception("pinot primary key columns must not be empty. Please specify the columns to be sorted.");
        }
      }
    }
...
//    pinotConfigsList.add(getVariable(req, "pinot-timestamp-columns"));
  ...
  private void handleRunReportalWithVariables(HttpServletRequest req, HashMap<String, Object> ret, Session session) throws ServletException, IOException {
    ...
    try {
      ...
      ret.put("result", "success");
      ret.put("redirect", "/pinot-reportal?view&logs&id=" + project.getId() + "&execid=" + exflow.getExecutionId());
    } catch (ExecutorManagerException e) {
      e.printStackTrace();
  ...
  private List<Variable> getPinotConfigs(HttpServletRequest req) throws ServletException {
  ...
  private Variable getVariable(HttpServletRequest req, String title) throws ServletException {
    Variable variable = new Variable();
