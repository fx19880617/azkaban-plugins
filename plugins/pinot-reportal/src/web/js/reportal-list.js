	...
	function displayData() {
		...
		if(displayReportals.length == 0) {
			...
			for (var i = 0; i < displayReportals.length; i++) {
				...
				var row = $("<tr></tr>");
				row.append("<td><a href='" + contextURL + "/pinot-reportal?view&id=" + id + "'>" + title + "</a>" + template + "</td>");
				row.append("<td>" + timeText + "</td>");
				row.append("<td>" + user + "</td>");
				row[0].item = displayReportals[i];

				row.find(".button-edit").eq(0).attr("href", contextURL + "/pinot-reportal?edit&id=" + id);
				row.find(".button-view").eq(0).attr("href", contextURL + "/pinot-reportal?view&id=" + id);
				row.find(".button-run").eq(0).attr("href", contextURL + "/pinot-reportal?run&id=" + id);
				...
				row.find(".button-delete").eq(0).click(function(event) {
					...
					if(confirm("Deletion confirmation", "Are you sure that you want to delete this reportal?")) {
						...
						$.ajax({
							url: contextURL + "/pinot-reportal?ajax=delete&id=" + item.id,
							dataType: "json"
						}).done(function(data) {
				...
				var bookmarkItem = row.find(".button-bookmark").eq(0).click(function(event) {
					...
					$.ajax({
						url: contextURL + "/pinot-reportal?ajax=bookmark&id=" + item.id,
						dataType: "json"
					}).done(function(data) {
						if(data.result == "success") {
				...
				var subscribeItem = row.find(".button-subscribe").eq(0).click(function(event) {
					...
					var email = "";
					$.ajax({
						url: contextURL + "/pinot-reportal?ajax=subscribe&id=" + item.id,
						dataType: "json"
					}).done(function(data) {
						if(data.result == "success") {
